  ...
  try {
    ...
    if (role !== "super_admin") {
      return NextResponse.json(
        { error: "Forbidden: Only Super Admins can send email campaigns" },
        { status: 403 },
      );
    }
    ...
    if (!subject || !headingMessage) {
      return NextResponse.json(
        { error: "Subject and heading message are required." },
        { status: 400 },
      );
    }
    ...
    if (!subscribers || subscribers.length === 0) {
      return NextResponse.json(
        {
          error:
            "No subscribers found. Get started by signing up some emails first!",
        },
        { status: 400 },
      );
    }
    ...
    const emailHtml = `
      <div style="font-family: 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; padding: 30px; max-width: 600px; margin: auto; border: 1px solid #e5e7eb; border-radius: 20px; background-color: #ffffff;">
        <div style="text-align: center; margin-bottom: 25px;">
          <h2 style="color: #6366f1; font-weight: 800; font-size: 28px; margin: 0; letter-spacing: -0.025em;">diversified-y&p</h2>
        ...
        <div style="margin-top: 30px; border-top: 1px solid #f3f4f6; padding-top: 20px; text-align: center;">
          <p style="font-size: 11px; color: #9ca3af; margin: 0;">
            You are receiving this because you subscribed to Diversified Y&P updates.
          </p>
    ...
    const mailOptions = {
      from: `"Diversified Y&P Deals" <${process.env.GMAIL_USER}>`,
    ...
    return NextResponse.json({
      success: true,
      message: `Email campaign sent to ${recipientEmails.length} subscribers!`,
    });
  } catch (error: any) {
