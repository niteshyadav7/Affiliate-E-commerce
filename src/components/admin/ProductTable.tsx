...
function BulkUploadModal({ isOpen, onClose, onUploadComplete }: { isOpen: boolean; onClose: () => void; onUploadComplete: () => void }) {
  ...
  const [sampleRowCopiedIdx, setSampleRowCopiedIdx] = useState<number | null>(null);
  ...
  const SCHEMA_COLUMNS = [
    ...
    { 
      ...
      example: 'Amazon|https://amazon.com/dp/B09XS7JWHH;Shopverse Buy|https://shopverse.com/buy', 
    ...
    { 
      ...
      example: 'This 100% pure cotton co-ord set for women features a beautifully styled short Kurti with fully breathable Palazzo bottoms. Tailored from premium fabrics suitable for casual summer wear and official occasions.', 
    ...
    { 
      ...
      example: '100% Breathable Cotton|Includes Kurti and Palazzo|Plus sizes from S to 7XL|Elegant green dye', 
    ...
    { 
      ...
      example: '{"Material": "Cotton", "Pattern": "Solid", "Fit": "Plus Size Available", "Wash Care": "Hand Wash"}', 
    ...
    { 
      ...
      example: 'https://lh3.googleusercontent.com/d/1_IMAGE_1|https://lh3.googleusercontent.com/d/1_IMAGE_2', 
    ...
    { 
      ...
      example: "Women's Premium Cotton Short Kurta Palazzo Set | Shopverse", 
    ...
    { 
      ...
      example: "Buy premium women's cotton short kurta palazzo sets on Shopverse today.", 
  ...
  const sampleRows = [
    [
      ...
      'Amazon|https://amazon.com/dp/B09XS7JWHH;Shopverse Buy|https://shopverse.com/buy',
      ...
      "Women's Premium Cotton Short Kurta Palazzo Set | Shopverse",
      "Buy premium women's cotton short kurta palazzo sets on Shopverse today."
    ],
    [
      'Acoustic Pro Max',
      'Electronics',
  ...
  const downloadSampleCSV = () => {
    const headers = SCHEMA_COLUMNS.map(col => `${col.name} (${col.required ? 'Required' : 'Optional'})`);
  ...
  const copySampleRowToClipboard = (idx: number) => {
    ...
    const rowString = rowData.map(val => `"${val.replace(/"/g, '""')}"`).join(',');
    ...
    reader.onload = (event) => {
      try {
        ...
        if (nameIndex === -1 || priceIndex === -1) {
          setErrors(['CSV must contain at least "Name" and "Price" columns. Make sure headers are unmodified.']);
          return;
        }
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          if (ratingIndex > -1) details.rating = parseFloat(row[ratingIndex]?.trim() || '5.0');
          if (reviewsIndex > -1) details.reviews_count = parseInt(row[reviewsIndex]?.trim() || '0');
        ...
        <div className="p-6 pb-4 flex justify-between items-center border-b border-surface-container-low flex-shrink-0 bg-gray-50/20">
          <div>
            <h3 className="font-display text-xl font-bold text-primary flex items-center gap-2">
              <Upload className="w-5 h-5 text-accent-coral animate-pulse" /> Bulk Product Upload Portal
            </h3>
            <p className="text-[11px] font-body text-secondary mt-0.5">Hydrate thousands of fashion sets, accessories, and links in seconds.</p>
          </div>
          <button 
        ...
        <div className="p-6 overflow-y-auto space-y-5 flex-1 pr-4 bg-white">
          {success ? (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <h4 className="font-display text-xl font-bold text-primary">Import Completed!</h4>
              <p className="text-sm font-body text-secondary max-w-sm mt-2">
                Successfully uploaded and hydrated <span className="text-primary font-bold">{parsedProducts.length} products</span> into the Shopverse inventory.
              </p>
            </div>
          ) : activeTab === 'upload' ? (
            <>
              ...
              <div className="bg-gradient-to-r from-surface-container-low to-surface-container/60 border border-surface-container p-4 rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 transition-all hover:shadow-sm">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="bg-accent-lime/20 text-primary border border-accent-lime font-display text-[9px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider">Self-Documenting CSV</span>
                    <h4 className="text-xs font-bold text-primary">Need the official bulk schema?</h4>
                  </div>
                  <p className="text-[11px] font-body text-secondary max-w-md leading-relaxed">
                    Download our beautiful template loaded with detailed examples. The parser is robust, handles column tips, and translates Google Drive images flawlessly.
                  </p>
                </div>
                <div className="flex gap-2 w-full sm:w-auto">
                  ...
                  >
                    <FileText className="w-3.5 h-3.5 text-accent-coral" /> Download CSV Template
                  </Button>
                </div>
              </div>
                ...
                {file ? (
                  <div className="space-y-2.5">
                    ...
                    <p className="text-[11px] font-mono text-secondary">
                      ({(file.size / 1024).toFixed(1)} KB) • File Loaded Successfully
                    </p>
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-primary">
                        Drag & Drop CSV file here, or <span className="text-accent-coral hover:underline">browse files</span>
                      </p>
                      <p className="text-[10px] font-body text-secondary mt-1 max-w-[340px] mx-auto leading-relaxed">
                        Supports standard comma-separated files. Highly complex structures (embedded redirects, spec JSONs) auto-hydrate.
                      </p>
                    </div>
                  </div>
                )}
              ...
              {errors.length > 0 && (
                <div className="bg-red-50 border border-red-100 rounded-2xl p-4 space-y-2.5 animate-in slide-in-from-top-2 duration-200">
                  <div className="flex items-center gap-2 text-red-700 font-bold text-xs">
                    <AlertCircle className="w-4 h-4 text-red-500" /> Validation Conflicts ({errors.length}):
                  </div>
                ...
                <div className="space-y-3 animate-in slide-in-from-bottom-2 duration-300">
                  <div className="flex items-center justify-between">
                    <h4 className="text-xs font-bold text-primary flex items-center gap-2">
                      ...
                      <span className="bg-primary text-white px-2 py-0.5 rounded-full font-mono text-[10px] font-extrabold">{parsedProducts.length}</span>
                    </h4>
                    <span className="text-[10px] font-body text-secondary">Verify loaded entries below before pushing to cloud</span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-gray-50 p-3 rounded-2xl border border-surface-container text-center">
                    <div className="p-2 bg-white rounded-xl border border-outline/5">
                      <div className="text-[9px] font-bold text-secondary uppercase">Products</div>
                      <div className="text-sm font-extrabold text-primary font-mono">{parsedProducts.length}</div>
                    </div>
                    <div className="p-2 bg-white rounded-xl border border-outline/5">
                      <div className="text-[9px] font-bold text-secondary uppercase">With Images</div>
                      <div className="text-sm font-extrabold text-green-600 font-mono">{parsedProducts.filter(p => p.image_url).length}</div>
                    </div>
                    <div className="p-2 bg-white rounded-xl border border-outline/5">
                      <div className="text-[9px] font-bold text-secondary uppercase">With Redirects</div>
                      <div className="text-sm font-extrabold text-primary font-mono">{parsedProducts.filter(p => p.links?.length > 0).length}</div>
                    </div>
                    <div className="p-2 bg-white rounded-xl border border-outline/5">
                      <div className="text-[9px] font-bold text-secondary uppercase">Avg Price</div>
                      <div className="text-sm font-extrabold text-accent-coral font-mono">
                        ${(parsedProducts.reduce((acc, curr) => acc + (parseFloat(curr.price) || 0), 0) / parsedProducts.length).toFixed(2)}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="border border-surface-container rounded-2xl bg-surface/10 divide-y divide-surface-container/60 max-h-[300px] overflow-y-auto pr-2 shadow-inner">
                    {parsedProducts.map((p, idx) => (
                      <div key={idx} className="p-4 hover:bg-white rounded-xl transition-all duration-150 flex justify-between items-start gap-6 border-b border-surface-container/10 last:border-b-0">
                        <div className="space-y-1.5 min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            <div className="text-xs font-bold text-primary truncate max-w-[280px]">{p.name}</div>
                          ...
                          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] font-body text-secondary leading-normal">
                            {p.image_url ? (
                              <span className="text-green-600 font-bold flex items-center gap-0.5">✓ Main Image</span>
                            ) : (
                              <span className="text-secondary opacity-60">✗ No Image</span>
                            )}
                            <span className="opacity-40">•</span>
                            <span>Links: <span className="font-bold text-primary">{p.links?.length || 0} redirection targets</span></span>
                            {p.details?.gallery_images && (
                              <>
                                <span className="opacity-40">•</span>
                                <span>Gallery Slider: <span className="font-bold text-primary">{p.details.gallery_images.length} photos</span></span>
                              </>
                            )}
                          </div>
                          {p.description && (
            ...
            <>
              ...
              <div className="space-y-4">
                <div className="space-y-1">
                  <h4 className="text-sm font-bold text-primary font-display">Interactive Schema Blueprint</h4>
                  <p className="text-[11px] font-body text-secondary leading-relaxed">
                    This interactive sheet replicates standard Excel spreadsheets. Click any column header to view detailed database schema formatting constraints, pro tips, and examples.
                  </p>
                </div>
                  ...
                  <div className="bg-gray-50 px-4 py-2 border-b border-surface-container flex items-center justify-between text-[10px] font-semibold text-secondary">
                    <div className="flex items-center gap-4">
                      <span className="flex items-center gap-1.5">
                        <span className="w-2.5 h-2.5 rounded-full bg-accent-coral border border-white shrink-0" /> Required Column
                      </span>
                      <span className="flex items-center gap-1.5">
                        <span className="w-2.5 h-2.5 rounded-full bg-accent-lime border border-white shrink-0" /> Optional Column
                      </span>
                    </div>
                    <span className="hidden sm:inline">💡 Click a column header to inspect details</span>
                  </div>
                    ...
                    <table className="w-full border-collapse text-left text-[11px] font-mono select-none">
                      <thead>
                        ...
                        <tr className="bg-gray-50 border-b border-surface-container">
                          ...
                          {SCHEMA_COLUMNS.map((_, idx) => (
                            <th key={idx} className="px-3 py-1 text-center font-bold text-secondary/50 border-r border-surface-container min-w-[155px]">
                        ...
                        <tr className="bg-white border-b border-surface-container-high">
                          <th className="px-2 py-2 text-center font-bold text-secondary border-r border-surface-container bg-gray-50">1</th>
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-center justify-between gap-2">
                                  ...
                                  {col.required ? (
                                    <span className={`text-[10px] font-black ${isSelected ? 'text-accent-coral' : 'text-accent-coral animate-pulse'}`}>*</span>
                                  ) : (
                                    <span className={`w-1.5 h-1.5 rounded-full ${isSelected ? 'bg-accent-lime' : 'bg-primary/30'}`} />
                                  )}
                                </div>
                              </th>
                            );
                      ...
                      <tbody className="divide-y divide-surface-container bg-white font-body">
                        ...
                        <tr className="hover:bg-primary/5 transition-colors">
                          <td className="px-2 py-2 text-center font-mono font-bold text-secondary border-r border-surface-container bg-gray-50">2</td>
                        ...
                        <tr className="hover:bg-primary/5 transition-colors">
                          <td className="px-2 py-2 text-center font-mono font-bold text-secondary border-r border-surface-container bg-gray-50">3</td>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                      <div className="space-y-2 flex-1">
                        ...
                        <div className="text-[10px] font-body text-secondary/80 flex items-start gap-1">
                          <span className="text-accent-coral font-bold shrink-0">Pro Tip:</span>
                      ...
                      <div className="bg-white border border-surface-container p-3 rounded-xl max-w-xs shrink-0 space-y-1 shadow-sm w-full md:w-auto">
                        <div className="text-[9px] font-bold text-secondary uppercase tracking-wider">Interactive Example Value</div>
                ...
                <div className="bg-gradient-to-r from-surface-container-low to-white border border-surface-container p-4 rounded-2xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                  <div className="space-y-0.5">
                    <h5 className="text-[11px] font-bold text-primary">Grab CSV Samples Direct</h5>
                    <p className="text-[10px] font-body text-secondary">Copy row presets for fashion sets or smart electronic devices to test uploads immediately.</p>
                  </div>
                  <div className="flex gap-2 flex-wrap w-full md:w-auto">
                    ...
                    >
                      {sampleRowCopiedIdx === 0 ? '✓ Copied Kurta Row!' : '📋 Copy Fashion Sample Row'}
                    </Button>
                    <Button 
                      type="button" 
                      variant="outline" 
                    ...
                    >
                      {sampleRowCopiedIdx === 1 ? '✓ Copied Electronics Row!' : '📋 Copy Electronics Sample Row'}
                    </Button>
                  </div>
                </div>
                ...
                <div className="border border-surface-container rounded-2xl p-4 bg-surface/30 space-y-3">
                  <h5 className="text-[10px] font-bold uppercase tracking-wider text-primary font-display flex items-center gap-1.5">
                    <AlertCircle className="w-3.5 h-3.5 text-accent-coral shrink-0" /> Essential Formatting Checklist
                  </h5>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2.5 text-[10px] font-body text-secondary leading-relaxed">
                    <div className="flex items-start gap-2">
                      <span className="bg-primary/5 font-mono px-1.5 py-0.5 rounded text-primary text-[9px] font-bold">Image URLs</span>
                      <span>Supports direct public URLs and Google Drive shares seamlessly.</span>
                    </div>
                    <div className="flex items-start gap-2">
                      <span className="bg-primary/5 font-mono px-1.5 py-0.5 rounded text-primary text-[9px] font-bold">Multiple Retailers</span>
                      <span>Redirect URLs are entered as: <code className="bg-white px-1 border border-outline/10 text-primary">Label|URL</code> separated by semicolons (;).</span>
                    </div>
                    <div className="flex items-start gap-2">
                      <span className="bg-primary/5 font-mono px-1.5 py-0.5 rounded text-primary text-[9px] font-bold">Slider Photos</span>
                      <span>Specify additional Gallery Images using absolute URLs separated by pipes (|).</span>
                    </div>
                    <div className="flex items-start gap-2">
                      <span className="bg-primary/5 font-mono px-1.5 py-0.5 rounded text-primary text-[9px] font-bold">JSON Specifications</span>
                      <span>Format as standard JSON, or key-value couples: <code className="bg-white px-1 border border-outline/10 text-primary">Color:Green|Fabric:Cotton</code></span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="p-4 px-6 border-t border-surface-container-low flex justify-between items-center flex-shrink-0 bg-gray-50/50">
            <div className="text-[10px] font-body text-secondary">
              {activeTab === 'upload' && parsedProducts.length > 0 && (
                <span className="text-green-600 font-bold">✓ Validation successful. Ready to import.</span>
              )}
            </div>
